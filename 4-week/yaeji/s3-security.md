...
> 💡 **SSE-KMS에서 특정 KMS 키 사용을 강제하려면?**
>
> S3 객체 업로드 시 **특정 KMS 키로 암호화**를 강제하려면, 요청 헤더에  
> `x-amz-server-side-encryption: aws:kms`  
> 와  
> `x-amz-server-side-encryption-aws-kms-key-id: <KMS Key ID>`  
> 를 반드시 포함해야 합니다.
>
> - `x-amz-server-side-encryption` 헤더만 강제하면, SSE-S3(AES256) 등 다른 암호화 방식도 허용될 수 있으므로 **KMS 키 강제에는 불충분**합니다.
> - 반드시 `x-amz-server-side-encryption-aws-kms-key-id` 헤더를 정책에서 체크해야, **특정 KMS 키로만 암호화**를 강제할 수 있습니다.
>
> 예시 버킷 정책:
> ```json
> {
...
> }
> ```
> - 위 정책은 **SSE-KMS와 특정 KMS 키 사용을 모두 강제**합니다.
> - `s3:PutObject` 작업에 적용해야 하며, `s3:PostObject`는 일반적으로 S3 객체 업로드에 사용되지 않습니다.
>
> ⚠️ **정리:**  
> - KMS 키 강제: `x-amz-server-side-encryption-aws-kms-key-id` 헤더 필요  
> - 단순 암호화 강제: `x-amz-server-side-encryption` 헤더만으로는 불충분  
> - 올바른 작업(`s3:PutObject`)에 정책을 적용해야 효과적입니다.
...
## 🔥 S3 Object Lambda의 추가 설명 및 실무 활용

### S3 Object Lambda의 동작 원리와 장점

- **S3 Object Lambda**는 S3 GET 요청 시 Lambda 함수를 통해 객체 데이터를 실시간으로 변환하거나 필터링할 수 있는 기능입니다.
- 별도의 데이터 사본을 만들 필요 없이, **요청 시점에만** 데이터가 변환되어 반환됩니다.
- 예를 들어, 이력서 파일에서 PII(개인 식별 정보)를 삭제해야 할 때, Lambda 함수가 S3에서 원본 객체를 읽어 PII를 제거한 후, 애플리케이션에 반환합니다.
- **저장 비용 절감**: 삭제된 사본을 별도로 저장하지 않으므로 스토리지 비용이 증가하지 않습니다.
- **운영 효율성**: 데이터 변환 로직이 Lambda 함수에 집중되어 관리가 쉽고, 데이터가 변경될 때마다 사본을 새로 만들 필요가 없습니다.

### S3 Object Lambda가 PII 삭제에 적합한 이유

- **실시간 변환**: 데이터가 요청될 때마다 Lambda 함수가 동작하여, 항상 최신 데이터에 대해 PII 삭제가 적용됩니다.
- **프록시 불필요**: 내부 서비스가 S3 Object Lambda Access Point를 통해 직접 데이터를 요청하면, 별도의 프록시 서버 없이도 변환된 데이터를 받을 수 있습니다.
- **비용 효율성**: 삭제된 사본을 저장하지 않으므로, 스토리지 비용이 증가하지 않습니다.
- **보안성**: Lambda 함수 내에서 PII 삭제 로직을 구현하므로, 민감 정보가 외부로 노출되지 않습니다.

### 다른 대안과의 비교

- **CloudFront + Lambda@Edge**:  
  - Lambda@Edge는 CDN 엣지에서 동작하여 최종 사용자에게 빠른 응답을 제공하는 데 적합합니다.
  - 그러나 내부 서비스에서 데이터 처리 전 PII 삭제가 필요할 때는 적합하지 않으며, 실행 비용도 더 높을 수 있습니다.
- **AWS Glue**:  
  - Glue는 대규모 ETL(추출, 변환, 적재) 작업에 적합합니다.
  - Glue로 PII를 삭제하면 삭제된 사본을 별도로 저장해야 하므로, 스토리지 비용이 증가합니다.
  - 단순 PII 삭제에는 과도한 솔루션입니다.
- **별도 S3 사본 생성**:  
  - Lambda 함수로 삭제된 사본을 별도 S3 버킷에 저장하면, 관리가 복잡해지고 저장 비용이 증가합니다.
  - S3 Object Lambda는 이러한 사본 생성 없이 실시간 변환이 가능합니다.

### 실무 활용 예시

- **PII 삭제**: 이력서, 로그, 고객 데이터 등에서 이름, 주민번호, 연락처 등 민감 정보를 실시간으로 삭제
- **행/필드 필터링**: 분석용 데이터에서 특정 필드만 추출하여 제공
- **이미지 변환**: 요청 시점에 이미지 크기 조정, 워터마크 삽입 등
- **포맷 변환**: XML → JSON 등 형식 변환

### 정책 및 보안

- S3 Object Lambda Access Point에는 별도의 IAM 정책과 액세스 포인트 정책을 적용할 수 있어,  
  **누가 어떤 데이터에 접근할 수 있는지 세밀하게 제어**할 수 있습니다.
- Lambda 함수 내에서 로깅, 모니터링, 예외 처리 등을 구현하여 보안 및 운영 품질을 높일 수 있습니다.
