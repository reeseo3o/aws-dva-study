...
*   **실행 역할 (Execution Role)**:
    ...
    *   **AWS 관리형 정책 예시**: `AWSLambdaBasicExecutionRole` (CloudWatch Logs), `AWSLambdaVPCAccessExecutionRole` (VPC), `AWSLambdaSQSQueueExecutionRole` (SQS 폴링), `AWSXRayDaemonWriteAccess` (X-Ray) 등.
...
이러한 제한을 초과하는 요구사항(예: 30GB RAM, 30분 실행 시간, 3GB 대형 파일)이 있다면, 해당 워크로드는 Lambda에 적합하지 않을 수 있습니다.

### 6. Lambda 모범 사례

*   **초기화 로직 최적화**: 데이터베이스 연결, AWS SDK 클라이언트 초기화, 대용량 종속성 로드 등은 **핸들러 함수 외부 (글로벌 스코프)**에서 수행하여 실행 컨텍스트가 재사용될 때 시간을 절약합니다 (콜드 스타트 이후 웜 스타트 시 성능 향상).
*   **환경 변수 활용**: 코드 내에 하드코딩하는 대신, 환경 변수를 사용하여 데이터베이스 연결 문자열, S3 버킷 이름 등 변경 가능한 설정을 관리합니다.
*   **민감 정보 보안**: 비밀번호나 API 키와 같은 민감한 정보는 AWS KMS를 사용하여 암호화한 후 환경 변수로 저장합니다.
*   **배포 패키지 최소화**: 런타임에 필요한 코드와 종속성만 포함하도록 패키지 크기를 줄입니다. 함수가 너무 크면 분리하는 것을 고려합니다.
*   **Lambda 계층 활용**: 여러 함수에서 공통으로 사용되는 라이브러리나 종속성은 계층으로 분리하여 재사용성을 높이고 배포 패키지 크기를 줄입니다.
*   **재귀 호출 방지**: Lambda 함수가 자기 자신을 직접 또는 간접적으로 계속 호출하는 재귀적 패턴은 의도치 않은 비용 증가와 스로틀링을 유발할 수 있으므로 피해야 합니다.
